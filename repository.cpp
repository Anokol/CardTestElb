...
QMap<QString, Videocard*> repository::get_video_cards()
{
    ...
    while(query.next()){
        ...
        QString ProcName= query.value(7).toString();
        QString DriverVersion= query.value(6).toString();
       ...
       TestInfo* info = new TestInfo(type, fps, frame_time, system, ProcName, DriverVersion);
       info->set_id(id);
       card->add(info);
    }
     ...
     while(it.hasNext()){
         it.next();
           ...
           for(TestInfo* info : infos){
               QSqlQuery query;
               query.prepare("INSERT INTO TestInfo (Type, FPS, FrameTime, System, VideoCardID, ProcName, DriverVersion )  VALUES (:Type, :FPS, :FrameTime, :System, :VideoCardID, :ProcName, :DriverVersion);");
               ...
               query.bindValue(":ProcName", info->get_ProcName());
               query.bindValue(":DriverVersion",info->get_driver());
